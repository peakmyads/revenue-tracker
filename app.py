...
def load_partner_list_from_gsheet():
    ...
    if not df.empty:
        ...
        df["Agreement Start Date"] = df["Agreement Start Date"].dt.strftime("%d-%b-%Y")
...
/* Target the logout button container */
div[data-testid="stHorizontalBlock"] div.stButton > button {

    background-color: #d11a2a !important;   /* RED */
    color: white !important;
...
div[data-testid="stHorizontalBlock"] div.stButton > button:hover {
    ...
    background-color: #65d778 !important;   /* YELLOW */
...
<div style="color:white;font-weight:700;font-size:18px;text-align:right;">
{"Login: " + st.session_state.get("user","") if st.session_state.get("logged_in") else ""}
</div>
...
if "Partner Onboarding Form" in tabs:
    with tabs["Partner Onboarding Form"]:
        ...
        if save_clicked:
            ...
            with st.spinner("Saving Partner..."):
                ...
                # Get existing partner short names (Column 3)
                existing_partners = worksheet.col_values(3)

                if short_name in existing_partners:
                    st.error("Partner already exists with this Short Name")
                    st.stop()
    ...
    with tabs["Summary"]:
        ...
        with row1_col1:
            ...
            if selected_partner == "Select Partner":
                ...
                if df_partner.empty:
                    ...
                    # Ensure month format
                    dsp_df["Month"] = pd.to_datetime(dsp_df["Month"], errors="coerce")
                    ssp_df["Month"] = pd.to_datetime(ssp_df["Month"], errors="coerce")
                    ...
                    for _, row in df_summary.iterrows():
                        ...
                        if not dsp_match.empty:
                            due_date = dsp_match.iloc[0]["Due Date"]
                            ...
                            if not ssp_match.empty:
                                due_date = ssp_match.iloc[0]["Due Date"]
                            else:
                                due_date = None

                        due_dates.append(due_date)

                    df_summary["Due Date"] = due_dates
                    
                    df_summary["Due Date"] = pd.to_datetime(df_summary["Due Date"], errors="coerce")

                    df_summary["Due Date"] = df_summary["Due Date"].dt.strftime("%d-%b-%Y")
        ...
        with row1_col2:
            ...
            with f2:
                ...
                month_list = ["All"] + months.strftime("%b-%Y").tolist()
            ...
            fy_start_date, fy_end_date = get_fy_date_range(selected_fy)

            df = df[
                (df["Month"] >= fy_start_date) &
                (df["Month"] <= fy_end_date)
            ]
            ...
            # Month filter overrides quarter
            ...
            if selected_month != "All":
                ...
                fx_rate = get_fx_rate(selected_month)
                ...
                fx_rate = sum(fx_values) / len(fx_values) if fx_values else 0
            ...
            df["C Net $"] = pd.to_numeric(df["C Net $"], errors="coerce").fillna(0)
            ...
            # Convert numeric
            df_cost["Amount USD"] = pd.to_numeric(df_cost["Amount USD"], errors="coerce").fillna(0)
            df_cost["Amount INR"] = pd.to_numeric(df_cost["Amount INR"], errors="coerce").fillna(0)
            df_cost["FX Rate"] = pd.to_numeric(df_cost["FX Rate"], errors="coerce").fillna(0)

            # Convert USD → INR if needed
            df_cost["Amount Final INR"] = df_cost["Amount INR"]

            usd_mask = df_cost["Currency"] == "USD"

            df_cost.loc[usd_mask, "Amount Final INR"] = (
                df_cost.loc[usd_mask, "Amount USD"] *
                df_cost.loc[usd_mask, "FX Rate"]
            )

            # FY filter
            df_cost_filtered = df_cost[df_cost["Financial Year"] == selected_fy]

            # Month filter
            if selected_month != "All":
                df_cost_filtered = df_cost_filtered[
                    df_cost_filtered["Month"] == selected_month
                ]
            ...
            if selected_quarter != "All":
                ...
                months_q = quarter_map[selected_quarter]

                df_cost_filtered = df_cost_filtered[
                    df_cost_filtered["Month"].str[:3].isin(months_q)
                ]

            st.divider()
            
            # Direct cost
            direct_cost = df_cost_filtered.loc[
                df_cost_filtered["Category"] == "Direct",
                "Amount Final INR"
            ].sum()

            # Indirect cost
            indirect_cost = df_cost_filtered.loc[
                df_cost_filtered["Category"] == "Indirect",
                "Amount Final INR"
            ].sum()
            ...
            gp_percent = (gross_profit / revenue_inr * 100) if revenue_inr != 0 else 0

            net_profit = gross_profit - indirect_cost

            np_percent = (net_profit / revenue_inr * 100) if revenue_inr != 0 else 0
            ...
            pnl_data = [
                ["Revenue", revenue_inr],
                ["Less: Direct Cost", -direct_cost],
                ["Gross Profit", gross_profit],
                ["GP %", gp_percent],
                ["Less: Indirect Cost", -indirect_cost],
            ...
            pnl_display.loc[
                pnl_display["Particulars"].str.contains("%"),
                "Amount"
            ] = pnl_display.loc[
                pnl_display["Particulars"].str.contains("%"),
                "Amount"
            ...
            def style_pnl(row):
                ...
                if "Gross Profit" in row["Particulars"]:
                    styles = ["font-weight:bold; background-color:#E8F5E9"] * 2

                if "Net Profit" in row["Particulars"]:
                    styles = ["font-weight:bold; background-color:#E3F2FD"] * 2

                if "%" in row["Particulars"]:
                    styles = ["font-weight:bold"] * 2

                # Negative values → red
                if isinstance(row["Amount"], str) and "-" in row["Amount"]:
                    styles[1] += "; color:#D32F2F; font-weight:bold"

                return styles

            pnl_display = pnl_display.reset_index(drop=True)
            
            styled_pnl = pnl_display.style.apply(style_pnl, axis=1).set_table_styles(
                [
                    {
                        "selector": "thead th",
                        "props": [
            ...
            <tr>
            <td><b>Revenue</b></td>
            <td style="text-align:right">₹{revenue_inr:,.0f}</td>
            </tr>

            <tr>
            <td>Less: Direct Cost</td>
            <td style="text-align:right; color:#D32F2F">₹{direct_cost:,.0f}</td>
            </tr>

            <tr style="background-color:#E0B0FF;">
            <td><b>Gross Profit</b></td>
            <td style="text-align:right"><b>₹{gross_profit:,.0f}</b></td>
            </tr>

            <tr>
            <td>GP %</td>
            <td style="text-align:right">{gp_percent:,.2f}%</td>
            </tr>

            <tr>
            <td>Less: Indirect Cost</td>
            <td style="text-align:right; color:#D32F2F">₹{indirect_cost:,.0f}</td>
            </tr>

            <tr style="background-color:#E0B0FF;">
            <td style="font-size:16px"><b>Net Profit</b></td>
            <td style="text-align:right; font-size:16px"><b>₹{net_profit:,.0f}</b></td>
            </tr>

            <tr>
            <td>NP %</td>
            <td style="text-align:right"><b>{np_percent:,.2f}%</b></td>
            </tr>
            ...
            <b>Revenue USD:</b> <b>${revenue_usd:,.2f}</b><br>
            <b>FX Used:</b> <b>{fx_rate:,.2f}</b>
    ...
    with tabs["Costs Centre"]:
        ...
        with col3:
            ...
            if st.button("🔄 Refresh", key="costcentre_refresh"):
                ...
                st.session_state.costcentre_df = load_cost_centre()
                st.rerun()
        ...
        with col4:
            ...
            if st.session_state.open_cost_popup:
                ...
                def add_cost_popup():
                    ...
                    # Add placeholder
                    month_options = ["Select"] + month_options
                    ...
                    if month != "Select":
                        fx_rate_auto = get_fx_rate(month)
                    else:
                        fx_rate_auto = 0.0
                    ...
                    if month:
                        ...
                        try:
                            ...
                            st.warning("Please Select Month to fetch FX rate")
                    ...
                    if st.button("Save Cost"):
                        ...
                        st.session_state.open_cost_popup = False   # ⭐ ADD THIS LINE
        ...
        # --------------------------------
        # REMOVE ANNUAL TOTAL FOR HEADER ROWS
        # --------------------------------
...
if "Admin Control" in tabs:
    ...
    with tabs["Admin Control"]:
        ...
        if st.session_state.role == "Admin":
            with st.expander("🔑 Admin Password Control"):
