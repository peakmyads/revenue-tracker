...
with tabs[7]:
    ...
    st.header("💰 Direct & Indirect Costs")

    # ==========================================
    # FINANCIAL YEAR LOGIC (CORRECTED)
    # ==========================================

    def generate_dynamic_fy():
        ...
        fy_list = [f"{start}-{str(start+1)[-2:]}"]

        # add next FY only if April has started
        if today >= datetime(start+1, 4, 1):
            next_year = start + 1
            fy_list.append(f"{next_year}-{str(next_year+1)[-2:]}")

        return fy_list

    fy_list = generate_dynamic_fy()
    ...
    month_cols = months.strftime("%b-%Y").tolist()

    base_columns = ["Particulars", "USD/INR"] + month_cols + ["Annual/FY Total"]
    ...
    structure = [
        ...
        ("PLL","direct"),
        ("+Row","add_direct"),
        ("Total USD","direct_total"),
        ("FX Rate","fx"),
        ("Total Direct Cost INR","direct_inr"),
        ("","blank"),
        ("Indirect Cost","header"),
        ("Salary","header"),
        ("Amit","indirect"),
        ("Sumit","indirect"),
        ("Ashutosh","indirect"),
        ...
        ("TDS","indirect"),
        ("Accounting","indirect"),
        ("CS Compliance","indirect"),
        ("Audit","indirect"),
        ("+Row","add_indirect"),
        ("","blank"),
        ("Other Office Expenses","header"),
        ("Rent","indirect"),
        ("+Row","add_indirect"),
        ("","blank"),
        ("Staff Welfare & Others","header"),
        ("Office Party","indirect"),
        ("+Row","add_indirect"),
        ("Total Indirect Cost INR","indirect_total")
    ]

    data = []
    for name, rtype in structure:
        row = {col: 0 for col in base_columns}
        row["Particulars"] = name
        row["USD/INR"] = ""
        row["row_type"] = rtype
        data.append(row)
    ...
    editable_js = JsCode("""
    function(params){
        let t = params.data.row_type;

        // Allow editing for cost rows
        if (t === "direct" || t === "indirect" || t === "fx") {
            return true;
        }
    ...
    function(params){
        ...
        let cur=params.data["USD/INR"];
        let sym=cur==="USD"?"$":"₹";
        return sym + parseFloat(params.value)
    ...
    total_logic = JsCode(f"""
    function(params) {{

        let api = params.api;
        let months = {month_cols};
        ...
        api.forEachNode(function(node) {{
            ...
            let type = node.data.row_type;

            if (type === "direct") {{
                months.forEach(function(col) {{
                    let val = parseFloat(node.data[col]) || 0;
                    directTotals[col] += val;
                }});
            }}
            ...
            if (type === "indirect") {{
                months.forEach(function(col) {{
                    let val = parseFloat(node.data[col]) || 0;
                    indirectTotals[col] += val;
                }});
            }}

            if (type === "fx") {{
                months.forEach(function(col) {{
                    let v = parseFloat(node.data[col]);
                    if (!isNaN(v) && v !== 0) {{
                        fx = v;
                    }}
                }});
        ...
        api.forEachNode(function(node) {{
            ...
            if (node.data.row_type === "direct_total") {{
                months.forEach(c => {{
                    node.setDataValue(c, directTotals[c]);
                }});
            }}

            if (node.data.row_type === "direct_inr") {{
                months.forEach(c => {{
                    node.setDataValue(c, directTotals[c] * fx);
                }});
            }}

            if (node.data.row_type === "indirect_total") {{
                months.forEach(c => {{
                    node.setDataValue(c, indirectTotals[c]);
                }});
            }}
        }});

        // 🔥 CRITICAL: Force refresh
    ...
    function(params){{
        ...
        // ONLY trigger when clicking Particulars column
        if (params.colDef.field !== "Particulars") return;

        let t = params.data.row_type;

        if(t === "add_direct" || t === "add_indirect"){{

            let months = {month_cols};

            let newRow = {{}};

            newRow["Particulars"] = "New Item";
            newRow["USD/INR"] = "";
            newRow["row_type"] = t === "add_direct" ? "direct" : "indirect";
            newRow["is_dynamic"] = true;  // 🔥 important

            months.forEach(c => newRow[c] = 0);
            ...
            // 🔥 Force refresh so edit works properly
            setTimeout(function(){{
    ...
    function(params){
        ...
        if(t==="header")
            return {backgroundColor:"#F2F2F2",fontWeight:"800"};

        if(t==="blank")
            return {backgroundColor:"#FAFAFA"};

        if(t==="direct_total" || t==="direct_inr")
            return {backgroundColor:"#003366",color:"white",fontWeight:"bold"};

        if(t==="indirect_total")
            return {backgroundColor:"#8B0000",color:"white",fontWeight:"bold"};
    ...
    gridOptions["onCellClicked"] = JsCode(f"""
    function(params){{

        // ADD ROW
        if (params.colDef.field === "Particulars") {{

            if (params.data.row_type === "add_direct" ||
                params.data.row_type === "add_indirect") {{
                ...
                let newRow = {{
                    ...
                    "USD/INR": "",
                    "row_type": params.data.row_type === "add_direct" ? "direct" : "indirect",
                    "is_dynamic": true,
                    "delete_btn": ""
                }};
        ...
        // DELETE ROW
        if (params.colDef.field === "delete_btn") {{

            if (params.data && params.data.is_dynamic === true) {{
                params.api.applyTransaction({{
    ...
    gb = GridOptionsBuilder.from_dataframe(df_cost)
    ...
    gb.configure_column(
        ...
        editable=JsCode("""
        function(params){
            return params.data.row_type === "direct" 
                || params.data.row_type === "indirect";
        }
        """)
    )
    
    gb.configure_column("USD/INR",
        editable=editable_js,
        cellEditor="agSelectCellEditor",
        cellEditorParams={"values":["USD","INR"]}
    )
    ...
    # Annual Total (FX row excluded)
    gb.configure_column(
        "Annual/FY Total",
        editable=False,
        valueGetter=JsCode(f"""
        function(params){{
            if(params.data.row_type==="fx") return "";
            let cols={month_cols};
            let sum=0;
            cols.forEach(c=>sum+=parseFloat(params.data[c]||0));
            return sum;
        }}
        """),
        valueFormatter=currency_formatter
    ...
    gb.configure_column("is_dynamic", hide=True)
    
    gridOptions=gb.build()
    gridOptions["rowSelection"] = "none"
    gridOptions["onCellValueChanged"]=total_logic
    gridOptions["onCellClicked"]=add_row_js
    ...
    grid_response = AgGrid(
        ...
        height=600,
        custom_css=custom_css,
        update_mode=GridUpdateMode.NO_UPDATE   # 🔥 THIS IS KEY
    )

    # ==========================================
    # SAFE SAVE (NO EXTRA READS)
    # ==========================================

    if st.button("💾 Save Data"):

        worksheet = spreadsheet.worksheet("PandL")

        if "PandL" in worksheets:
            worksheet = worksheets["PandL"]
        else:
            worksheet = spreadsheet.worksheet("PandL")

        import numpy as np
        
        df_save = pd.DataFrame(grid_response["data"])
        
        # Auto-set row_type for new rows
        df_save.loc[
            df_save["row_type"] == "add_direct",
            "row_type"
        ] = "direct"

        # Mark dynamic rows
        df_save.loc[
            df_save["Particulars"].notna() &
            (df_save["Particulars"] != "") &
            (df_save["row_type"] == ""),
            "row_type"
        ] = "direct"
        ...
        df_save.loc[
            df_save["row_type"] == "direct",
            "is_dynamic"
        ] = 1
        
        df_save = clean_for_gsheet(df_save)
        ...
        st.success("Saved to Google Sheet (PandL)")